        ...
        <div class="flex-1 overflow-y-auto">
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-4">
                @forelse ($products as $item)
                    <div class="border rounded-lg border-gray-200 p-3 hover:shadow-lg cursor-pointer transition flex flex-col justify-between relative bg-white">
                        ...
                        <span class="absolute top-2 right-2 bg-blue-100 text-blue-800 text-xs font-semibold px-2 py-1 rounded">
                            Stock: {{ $item->quantity }}
                        </span>
...
function renderCart(items, total) {
    ...
    if (items.length === 0) {
        ...
        cartContainer.innerHTML = items.map(item => `
            <div class="bg-gray-50 rounded-lg p-3 flex items-center gap-3 hover:bg-gray-100 transition">
                ...
                <div class="flex items-center gap-1">
                    <button onclick="updateQuantity(${item.cart_item_id}, ${item.product_id}, ${item.quantity - 1})" 
                            class="w-7 h-7 bg-gray-200 rounded hover:bg-gray-300 flex items-center justify-center transition"
                    ...
                    <input type="number" 
                           ...
                           class="w-14 text-center border text-gray-800 border-gray-300 rounded px-1 py-1 text-sm focus:ring-2 focus:ring-green-500 focus:outline-none"
                           min="1"
                           title="Quantity">
                    <button onclick="updateQuantity(${item.cart_item_id}, ${item.product_id}, ${item.quantity + 1})" 
                            class="w-7 h-7 bg-gray-200 rounded hover:bg-gray-300 flex items-center justify-center transition"
                ...
                <button onclick="removeFromCart(${item.cart_item_id}, ${item.product_id})" 
                        class="text-red-500 hover:text-red-700 ml-1 transition"
                        title="Remove item">
...
function checkout() {
    ...
    showConfirmModal(
        ...
        () => {
            processCheckout(); // call actual checkout
        }
    );
}
