...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $todaySales = Sales::whereDate('date', Carbon::today())->count();
        $todayRevenue = Sales::whereDate('date', Carbon::today())
            ->where('status', 'completed')
            ->sum('total_amount');

        // Monthly Sales Statistics
        $monthlySales = Sales::whereMonth('date', Carbon::now()->month)
            ->whereYear('date', Carbon::now()->year)
            ->where('status', 'completed')
            ->sum('total_amount');

        // Transaction Breakdown (This Month)
        $stockInCount = Transactions::whereMonth('transaction_date', Carbon::now()->month)
            ->whereYear('transaction_date', Carbon::now()->year)
            ->count();

        $stockOutCount = Transactions::whereMonth('transaction_date', Carbon::now()->month)
            ->whereYear('transaction_date', Carbon::now()->year)
            ->count();
        ...
        $recentSales = Sales::with('user')
            ->orderBy('date', 'desc')  // FIXED: changed from created_at to date
            ->limit(5)
            ->get();
