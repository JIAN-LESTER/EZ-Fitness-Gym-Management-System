...
class UserManagementController extends Controller
{
    ...
    public function viewUsers(Request $request)
    {
        ...
        $plans = MembershipPlan::select('plan_id', 'name', 'price')->get();
    ...
    public function store(Request $request)
    {
        ...
        if ($user->role === 'member' && ($request->has('plan_id') || $request->has('sex'))) {
            $plan = $request->has('plan_id') ? MembershipPlan::find($validated['plan_id']) : null;
            $subscription = $request->has('subscription_id') ? Subscriptions::find($validated['subscription_id']) : null;
    ...
    public function getPlansByBranch(Request $request)
    {
        ...
        $plans = MembershipPlan::where('branch_id', $branchId)
            ->orWhereNull('branch_id') // global plans
            ->select('plan_id', 'name', 'price')
            ->get();

        $subscriptions = Subscriptions::where('branch_id', $branchId)
    ...
    public function edit($id)
    {
        ...
        if ($user->member) {
            $response['member'] = [
                ...
                'sex' => $user->member->sex,
                'birthday' => $user->member->birthday ? Carbon::parse($user->member->birthday)->format('Y-m-d') : null,
    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($user->role === 'member') {
            ...
            } elseif ($request->has('plan_id') && $request->has('subscription_id')) {
                $plan = MembershipPlan::find($validated['plan_id']);
                $subscription = Subscriptions::find($validated['subscription_id']);
                $paymentMethod = $validated['payment_method'] ?? 'cash';
                $referenceCode = $validated['reference_code'] ?? null;
    ...
    public function approveSubscription(Request $request, $memberId)
    {
        try {
            ...
            try {
                ...
                if ($isRenewal && $member->end_date_for_subscription && Carbon::parse($member->end_date_for_subscription)->isFuture()) {
                    $newSubscriptionEndDate = Carbon::parse($member->end_date_for_subscription)
                ...
                // For plan end date
                if ($isRenewal && $member->end_date && Carbon::parse($member->end_date)->isFuture()) {
                    $newPlanEndDate = Carbon::parse($member->end_date)
    ...
    public function suspendMember($memberId)
    {
        try {
            ...
            if ($member->end_date_for_subscription) {
                $endDate = Carbon::parse($member->end_date_for_subscription);
